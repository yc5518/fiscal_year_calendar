 ...
 */
function addHolidaySet(countryCode: CountryCode): Holiday[] {
    ...
    case "US":
        ...
        const mlkDay = moment().year(currentYear).month(0).date(1);
        while (mlkDay.day() !== 1) {
            mlkDay.add(1, "day");
        }
        ...
        const presidentsDay = moment().year(currentYear).month(1).date(1);
        ...
        const memorialDay = moment().year(currentYear).month(4).endOf("month");
        ...
        const laborDay = moment().year(currentYear).month(8).date(1);
        ...
        const columbusDay = moment().year(currentYear).month(9).date(1);
        ...
        const thanksgivingDay = moment().year(currentYear).month(10).date(1);
    ...
    case "UK":
        ...
        const earlyMayBankHoliday = moment().year(currentYear).month(4).date(1);
        ...
        const springBankHoliday = moment().year(currentYear).month(4).endOf("month");
        ...
        const summerBankHoliday = moment().year(currentYear).month(7).endOf("month");
 ...
 */
function calculateEasterSunday(year: number): moment.Moment {
 ...
 */
function isHoliday(date: string | Date | moment.Moment): boolean {
    const checkDate = moment.isMoment(date) ? date.clone() : moment(date);
 ...
 */
function getBusinessDaysInPeriod(
