...
// With customised locale, if we set first day of a week to be Sunday, 0 represents **Sunday**; if we set fist day of a weeks to be Monday, 0 represents **Monday**.
const FIRST_DAY_NUM_OF_WEEK = 0;

const REPORT_TYPE_START_OF_WEEK = {
    monday: {
        value: "monday",
        label: "Monday",
    },
...
const selectOptionStructureWithLabel = ((value, label) => ({
...
function getCurrentWeekMoment(timezone = null, year = null, startOfWeek = null) {
    ...
    // dayInFirstWeek is the day must be included in first week of FY. E.g.: Starbucks's will be October 1
    const dayInFirstWeek = currentWeek.clone().month(9).startOf("month");
    // startDateOfFY must be start date of the week which contains dayInFristWeek, for starbucks,
...
function buildWeekOptions(timezone = null, year = null, startOfWeek = REPORT_TYPE_START_OF_WEEK.monday.value) {
    ...
    return weeks.map((value) => {
        ...
        return selectOptionStructureWithLabel(String(value), `Week ${value} (${currentWeek.startOf("week").format(format)} - ${currentWeek.endOf("week").format(format)})`);
    });
}
