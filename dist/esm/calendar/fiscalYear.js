...
import { DEFAULT_FY_START_MONTH, DEFAULT_FY_START_DAY, FIRST_DAY_NUM_OF_WEEK, START_OF_WEEK, CALENDAR_SYSTEMS } from "../utils/constants";
/**
 ...
 */
function calFiscalYear(date, timezone = null, fyStartMonth = DEFAULT_FY_START_MONTH, fyStartDay = DEFAULT_FY_START_DAY) {
    const dateMoment = moment.isMoment(date) ? date.clone() : (timezone ? moment.tz(String(date), timezone) : moment(String(date)));
 ...
 */
function getCurrentWeekMoment(timezone = null, year = null, startOfWeek = null, fyStartMonth = DEFAULT_FY_START_MONTH, fyStartDay = DEFAULT_FY_START_DAY) {
    // We are actually using currentWeek to get current year
    const currentWeek = timezone ? moment.tz(timezone) : moment();
    if (year) {
    ...
    const dow = getDayOfWeek(startOfWeek || START_OF_WEEK.monday.value);
 ...
 */
function calStartDate(timezone = null, year = null, startOfWeek = null, fyStartMonth = DEFAULT_FY_START_MONTH, fyStartDay = DEFAULT_FY_START_DAY) {
 ...
 */
function calEndDate(timezone = null, year = null, startOfWeek = null, fyStartMonth = DEFAULT_FY_START_MONTH, fyStartDay = DEFAULT_FY_START_DAY) {
    const currentTime = timezone ? moment.tz(timezone) : moment();
    if (year) {
        currentTime.year(year);
    }
 ...
 */
function buildWeekOptions(startOfWeek = START_OF_WEEK.monday.value, timezone = null, year = null, fyStartMonth = DEFAULT_FY_START_MONTH, fyStartDay = DEFAULT_FY_START_DAY, calendarSystem = CALENDAR_SYSTEMS.STANDARD) {
 ...
 */
function buildQuarterOptions(startOfWeek = START_OF_WEEK.monday.value, timezone = null, year = null, fyStartMonth = DEFAULT_FY_START_MONTH, fyStartDay = DEFAULT_FY_START_DAY, calendarSystem = CALENDAR_SYSTEMS.STANDARD) {
    const startDate = calStartDate(timezone, year, startOfWeek, fyStartMonth, fyStartDay);
    const quarters = [];
    for (let i = 1; i <= 4; i++) {
 ...
 */
function buildMonthOptions(startOfWeek = START_OF_WEEK.monday.value, timezone = null, year = null, fyStartMonth = DEFAULT_FY_START_MONTH, fyStartDay = DEFAULT_FY_START_DAY, calendarSystem = CALENDAR_SYSTEMS.STANDARD) {
    const startDate = calStartDate(timezone, year, startOfWeek, fyStartMonth, fyStartDay);
    const months = [];
    for (let i = 0; i < 12; i++) {
 ...
 */
function calDateNum(date, timezone = null, startOfWeek = START_OF_WEEK.monday.value, fyStartMonth = DEFAULT_FY_START_MONTH, fyStartDay = DEFAULT_FY_START_DAY) {
    const dateMoment = moment.isMoment(date) ? date.clone() : (timezone ? moment.tz(String(date), timezone) : moment(String(date)));
 ...
 */
function calFiscalQuarter(date, timezone = null, startOfWeek = START_OF_WEEK.monday.value, fyStartMonth = DEFAULT_FY_START_MONTH, fyStartDay = DEFAULT_FY_START_DAY) {
    const dateMoment = moment.isMoment(date) ? date.clone() : (timezone ? moment.tz(String(date), timezone) : moment(String(date)));
    const fiscalYear = calFiscalYear(dateMoment, timezone, fyStartMonth, fyStartDay);
    const startDate = calStartDate(timezone, fiscalYear, startOfWeek, fyStartMonth, fyStartDay);
 ...
 */
function calFiscalMonth(date, timezone = null, startOfWeek = START_OF_WEEK.monday.value, fyStartMonth = DEFAULT_FY_START_MONTH, fyStartDay = DEFAULT_FY_START_DAY) {
    const dateMoment = moment.isMoment(date) ? date.clone() : (timezone ? moment.tz(String(date), timezone) : moment(String(date)));
    const fiscalYear = calFiscalYear(dateMoment, timezone, fyStartMonth, fyStartDay);
    const startDate = calStartDate(timezone, fiscalYear, startOfWeek, fyStartMonth, fyStartDay);
    const monthDiff = dateMoment.diff(startDate, "months");
